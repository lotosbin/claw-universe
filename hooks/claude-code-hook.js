...
function mapStatus(hookEvent) {
  switch (hookEvent) {
    case 'SessionStart': return { status: 'idle', message: '会话启动' };
    case 'UserPromptSubmit': return { status: 'thinking', message: '处理中' };
    case 'PreToolUse': return { status: 'active', message: '使用工具' };
    case 'PostToolUse': return { status: 'active', message: '工具完成' };
    case 'Stop': return { status: 'idle', message: '执行完成' };
    case 'SessionEnd': return { status: 'idle', message: '会话结束' };
    default: return { status: 'idle', message: hookEvent };
  }
}
